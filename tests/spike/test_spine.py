...
def test_s01_straight_fence_spans_and_posts(knowledge, catalog):
    ...
    line_stations = sorted(p.station_mm for p in posts if p.kind == "line")
    assert line_stations == [1500, 3000, 4500]
    assert all(p.sku == "POST-S" for p in posts)
    assert all(p.mounting == "ground" for p in posts)
    assert all(s.width_mm <= 1800 for s in spans)
...
def test_s08_demand_exactly_at_package_boundary(knowledge, catalog):
...
def test_full_spine_traceability(knowledge, catalog):
    ...
    # reverse direction: no demand silently dropped by fulfillment
    covered = {p for line in bom.lines for p in line.pegs}
    covered |= {p for a in bom.allocations for p in a.pegs}
    assert covered == set(req_by_id)
