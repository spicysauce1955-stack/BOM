...
def test_a_supply_run_round_trips(store):
...
def test_an_unknown_supply_run_is_none_not_an_error(store):
    assert store.load_supply_run("sup_nope") is None


def test_the_store_stamps_created_at(store):
    store.save_supply_run(_sup())
    assert store.load_supply_run("sup_aaa").created_at


def test_saving_the_same_id_twice_does_not_write_twice(store):
...
def test_supply_runs_are_listed_per_design(store):
