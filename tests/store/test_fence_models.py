...
def test_a_draft_may_be_overwritten_in_place(store):
...
def test_a_published_version_refuses_to_be_rewritten(store):
...
def test_status_change_rewrites_the_document_too(store):
...
def test_unknown_status_and_unknown_model_are_refused(store):
    store.save_fence_model(model())
...
def test_library_is_every_row_ordered(store):
...
def test_next_version_starts_at_one_and_follows_the_maximum(store):
...
def test_a_fresh_store_is_seeded_with_the_demo_models(store):
    lib = store.fence_model_library()
...
def test_a_seeded_draft_keeps_its_status_and_leaves_the_line_where_it_was(store):
...
def test_seeding_never_overwrites_what_is_already_there(dsn):
    """Shape B: the thing under test is what a SECOND `Store` sees in the
