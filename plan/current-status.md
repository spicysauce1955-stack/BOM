...
`37da965`). Four concurrent agents: both project reviewers over item 8, and two
implementers closing the last three of *Still owed, and small* in their own
worktrees. Everything below is merged into `gap-as-a-return-type`.

**The gate's own proof procedure held across the merge**, and it is the claim
worth leading with: `tests/scenarios/compatibility_gate/` gained exactly ONE file
(`site_variant.json`) and **not one line changed in the fourteen existing ones**.
Adding a fixture rewrites all of them, so git reporting no change to any is the
proof, not a happy accident. Two agents predicted a cost that never came: the plan
had said closing the per-span defaults "moves golden numbers on runs that are
currently green", and a 20-case probe over five knowledge bases showed zero
differences in purchase qty, engineering qty, overage, unit price, line totals,
BOM totals, span widths, rail/screw/post counts and unresolved lines.

**The merge cost one conflict, and it was the honest kind.** Both implementers
struck their own entry in `plan/next-session.md` and left the other's text as they
found it, so the resolution was to take both strikes. Nothing semantic collided —
which is what the worktree discipline is for, and worth contrasting with the
2026-08-25 round's seven conflicts across three items that all rewrote one read
model.

**What the reviews cost, in one line each.** Neither reviewer found anything wrong
with the annexe, the placement table or the type; every finding was one layer out.
And the site agent's own reviewers found the same shape one level up: *the claim
its commit message called load-bearing had three docstrings asserting it and zero
assertions testing it* — the test that named the mutation it killed killed neither,
because its model had `post=None` and the code path was never reached. Two direct
mutations passed the entire suite.

**The browser gate then caught two of MY checks, not the code.** Both asserted the
structure sheet's annexe holds exactly one entry — written before the same session
taught it to carry the line notices as well, and unverifiable at the time because
a live agent held the ports. `.doc-warning` counted together is what made them
stale: an inlined entry is wrapped in `.doc-warnings` and labelled, a job-wide one
is not, and the checks now count the two separately and assert the product notice
IS on the sheet exactly once. A check that cannot survive the feature it guards
growing a legitimate second case was never testing the property.

**Two operational findings worth keeping, because they will recur.**

1. **Both implementer worktrees were cut at `42598c5` — an ancestor of the base I
   briefed** — and both agents caught it and fast-forwarded before editing. One of
   them names the consequence precisely: unchecked, the frost-depth fix would have
   been edited into a file where `SiteConditions` did not yet exist. The recorded
   trap said "check `git worktree list` before briefing anyone"; the sharper
   version is that the AGENT must verify its own base, because the parent's view
   of where a worktree was cut can be wrong.
2. **A concurrent agent's server made the shared tree's gates unreadable.** Ports
   8800 and 9400 were held by a live worktree, so `tests/tools` reported 22 errors
   and the browser suite could not start at all — while nothing was wrong. The
   errors were read as a collision rather than as a defect and the ports were NOT
   freed, because killing a running agent's server to get a green number is how a
   gate stops meaning anything. Both were re-run after.
...
The whole build order except items 6 and 7, which are off the table until the
Knowledge Platform publishes something.

- **9 — continuity is DERIVED.** `strategy/continuity.py`, pure, called once per
  run after layout and recorded on `Strategy.member_runs`; demand, the cut plan
  and the structure sheet all READ that one record, because a read model may
  never recompute a quantity. Demand emits one line per member run pegged to
  every bay it crosses, `qty` NOT multiplied by bays — a rail through three bays
  is one piece bought once. The model authors a CAPABILITY (`post_joint`) and the
  engine derives the behaviour; the authored `continuity` override still wins and
  is never silent. **Abstention is not disagreement**: a `through` member with no
  stated stock length is obligation 14's blessed case, so no disagreement fires —
  instead the run reports that nothing bounded the extent, which is the one
  branch that could otherwise invent a 120 m rail.
- **10 — containment, and the credit.** A credit is a **supply route, not a
  quantity**: the contained slot names what it supplies, the demanding slot
  carries `credited_qty` + `credited_by`, and `qty` is what remains. One positive
  BOM line, smaller — a −2 hinge line is unactionable and a silently reduced
  count is untraceable. Path keys (`gate_kit/hinge`) reuse `slot_key`'s existing
  identity rather than inventing one.
- **11 — step scopes and a partial order.** All five scopes authorable; `run` and
  `site` render as present-and-unrendered. `requires` is edges with a kind, and
  the read model returns ONE linearisation plus the shape — which steps share a
  stage, whether the sequence is unique, and whether the document asserted any
  dependency at all. It says "this is an order, not the order" rather than
  dressing print order up as a dependency.

**No existing job's cost moved.** Two fixtures were ADDED to the compatibility
gate (`kit.json`, `through_rail.json`) and not one of the twelve existing golden
files changed. Item 9 makes the point sharply: deriving from stock length ALONE
would have made nearly every fixture continuous — RAIL-3000 over two 1500 mm bays
is exactly 3000 mm — and would have been wrong, because an M-LEGACY rail is
screwed to the post face.

**Each agent ran its own reviewers, and the reviews earned their place.** Item 10's
critic found nesting under a credited slot double-counting (twelve washers for
four hinges that hold eight) and crediting a drawn slot breaking the elevation; its
test reviewer left fourteen surviving mutants, and writing one missing test found
a live defect neither reviewer had. Item 9's test reviewer found two inversions
that would have shipped green. Item 9 also **rejected** one critic finding with
evidence, which is the right outcome when the evidence is on your side.

**The merge was the expensive part**, and worth recording: seven conflicts across
three items that all rewrote the same read model and the same demand loop. Both
`derive.py` guards were needed (a contained slot is a member not a purchase; a
continuous member is bought once per run) and both `assembly.py` docstrings
claimed the OTHER item's limitation as outstanding — each had been closed by the
item it was merged with.
