...
| 4 | The IAP key grace window is not enforced between retries; keys past the declared boundary verify for up to 60s | ~~**Slice 4**, with finding 2~~ — **done in slice 3's PR #14 review instead**, because "up to 60s" was wrong: the gap never closed, so a multi-day key-endpoint outage served retired keys for 59 of every 60 seconds indefinitely. The deferral rested on that false reading. See the carried-findings note's Correction. |
