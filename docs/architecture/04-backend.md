...
73 routes. Grouped by what they are for rather than by path:
...
| Identity | `GET /api/session`, `GET /api/users`, `POST /api/dev/identity` | Who is asking, and whether they may ask at all. **Identity is Google's**: a provider behind `identity/ports.py` answers *who is this* from the transport (IAP's signed assertion in production, a cookie on a laptop), and the `users` table answers *what may they do*. A `capacity` (`sales \| backoffice \| admin`) is what an account may DO and is read on the server; a `view` is what is SHOWN and is a browser preference — `identity/model.py` carries the paragraph keeping the two apart. **Default-deny**: one dependency on the app resolves every caller to an active capacity row before any route runs, and refuses with `no_identity` (401) or `no_capacity` / `account_deactivated` / `subject_mismatch` (403). The exemptions are an explicit list in `api/auth.py` — `/api/health`, because an uptime check cannot hold a Google account, and `/api/session`, which is the one route that answers WITHOUT a capacity row: a person IAP let through but nobody has granted anything has to be nameable to the admin they are about to ask. It also says which view to open on, which is the safe way to flip the default the salesperson MVP left at `all` — nobody edits a global setting, Dana lands on her own screen because of who she is. `POST /api/dev/identity` is registered ONLY under `FENCEAI_IDENTITY=dev`, so under `iap` it does not exist to be found; it is an impersonation switch and is named as one. The static mount at `/` is a `Mount` rather than a route, so the gate never reaches it — which is what lets the refusal screen render itself in Hebrew for somebody the API refuses. The demo accounts are seeded ONLY under `dev`, and that condition is what makes `FENCEAI_BOOTSTRAP_ADMIN` able to fire at all: an unconditional `u_admin` put an admin row in every fresh database before the first request arrived, which left a real company's first admin with no way in. `/openapi.json`, `/docs` and `/redoc` are off outside `dev` — FastAPI registers them as plain `Route`s, so the gate cannot reach them, and left on they hand the whole API surface to anybody who reaches the service |
